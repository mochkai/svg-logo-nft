...
class AutoGen {
  ...
  public async generateJsonFiles(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      let jsonCheckAttempts = 0;

      let jsonUpdateCheck = setInterval(() => {

        console.log(`${jsonUpdateCount} == ${this.metadata.length}`);

        if (jsonCheckAttempts <= 6) {
          if (jsonUpdateCount == this.metadata.length) {
            clearInterval(jsonUpdateCheck);
            this.ipfs.files.stat(this.baseFolder + "/json", { hash: true })
              .then((result: any) => {
                resolve(result.cid.toString());
              })
              .catch((error: any) => {
                this.debugLog("Stat file error: ", error);
                reject("Error: There was an error retrieving the JSON path hash.");
              });
          } else {
            jsonCheckAttempts++;
          }
